function solve_TS_LP_DUAL(grb_env, data, line_vector::Array{Bool, 1}; threads = 1)
    ...
    TS = Model(with_optimizer(Gurobi.Optimizer, grb_env, Threads = threads, OutputFlag = 1))
    ...
    if status == MOI.TerminationStatusCode(1)
        ...
        @inline function get_dual_vector()

            dual_vector = Vector{Float64}()

            for i in 1:length(data.lines)
                if line_vector[i]
                    push!(dual_vector, dual.(alpha1)[i])
                else
                    push!(dual_vector, dual.(alpha2).data[i])
                end
            end
        ...
        dual_vector = get_dual_vector()
    end

    return time, status, objective, solution, dual_vector
end
