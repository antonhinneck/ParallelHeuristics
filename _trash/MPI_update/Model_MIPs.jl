function solve_TS_MIPs(data; heuristic = false, threads = 4, time_limit = 20, start = false)
    ...
    TS = Model(optimizer_with_attributes(() -> Gurobi.Optimizer(env_ts), "Heuristics" => 0.1, "TimeLimit" => time_limit, "Threads" => threads,  "OutputFlag" => 1))
    ...
    @inline function pscc_callback_rcv(cbdata::CallbackData, where::Cint)
        ...
        #if where == convert(Cint, Gurobi.CB_MIPSOL)
            #println(Gurobi.cbget_mipsol_sol(cbdata, where))
        #end
        ...
        #     get_switching_status!(switching_status, Gurobi.cbget_mipsol_sol(cbdata, where))
        #
        ...
        #                     for j in 1:(size - 1)
        #                         MPI.isend([cb_objective, cb_incumbent...], j, MSG_INCUMBENT, cw)
        #                     end
        #                 end
        #             end
        #         end
    ...
    theta_idx = [TS.moi_backend.model_to_optimizer_map[index(theta[data.buses[1]])].value,
                 TS.moi_backend.model_to_optimizer_map[index(theta[maximum(data.buses)])].value]
    ...
    println(string("JuMP model built: ", time() - t))
    t = Base.time()
    optimize!(TS)
