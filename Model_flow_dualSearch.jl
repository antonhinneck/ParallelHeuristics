function solve_TS_DualLP(data, line_capacity)
    ...
    #Line limit
    cref = @constraint(TS, power_flow_limit_1[l in data.lines], power_flow_var[l] == line_capacity[l])
    ...
    for i in 1:length(data.lines)
        try push!(duals, dual(cref[data.lines[i]]))
        catch KeyError
        end
    end
    ...
    #print(value.(theta).data)
        ...
        output = [te-t, JuMP.objective_value(TS), duals]
    end
