function build_stdcopf(ge::Gurobi.Env, data; outputflag = 0, threads = 1)

    m = Model(with_optimizer(Gurobi.Optimizer, ge))
    ...
    #Line limit
    @constraint(m, fl1[l in data.lines], f[l] <= data.line_capacity[l])
    @constraint(m, fl2[l in data.lines], f[l] >= -data.line_capacity[l])
