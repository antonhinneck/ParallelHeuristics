...
for i in 3:3
        ...
        if rank == ROOT
                ...
                terminated = Array{Bool, 1}(undef, size - 1)
                for i in 1:HRSTC_TIMELIMIT

                        for j in 1:(size - 1)
                                status, solutions = MPI.irecv(1, MSG_SOLUTIONS, cw)
                                MPI.isend(true, j, MSG_TERMINATE, cw)
                        end

                        status = true
                        for j in 1:length(terminated)
                                if !terminated[j]
                                        status = false
                                end
                        end
        ...
        #--------------------
                ...
                while (optimizer_terminated == false || optimizer_terminated == nothing && HRSTC)
                        ...
                        if !init
                                ...
                                time, status, objective, solution, dual_indicators = solve_TS_LP(grb_env, data, line_indicators, threads = 2)
                        ...
                        ## Obtain duals
                        ##------------------------
                        if init && optimizer_terminated != true && rank == 1

                                message, has_data = MPI.recv(ROOT, MSG_TERMINATE, cw)
                                ...
                                incumbent, has_data = MPI.recv(ROOT, MSG_INCUMBENT, cw)
                                if typeof(incumbent) != nothing && typeof(incumbent) != Symbol
                                        if  incumbent[1] < incumbent_obj
                                                incumbent_updated = true
                                                incumbent_obj = incumbent[1]
                                                incumbent_sol = incumbent[2:length(incumbent)]
                                ...
                                ##---------------------------
                                if optimizer_terminated != true && rank == 1
                                        ...
                                        time, status, has_values, objectives, solutions = solve_TS_MIP_Deco(grb_env, data, li, constraint_indicators, threads = 2, time_limit = HRSTC_TIMELIMIT)
                        ...
                        if init && optimizer_terminated != true && rank == 2

                                runtime, status, objective, solution, duals = solve_TS_LP_Duals(grb_env, data, line_indicators)
                                ...
                                plist = get_priority_list(line_idxs, 1, 60)

                                for i in 1:length(plist)
                                        constraint_indicators[plist[i]] = false
                                end

                                time, status, has_values, objectives, solutions = solve_TS_MIP_Deco(grb_env, data, line_indicators, constraint_indicators, threads = 2, time_limit = HRSTC_TIMELIMIT)
